...
async def receive_issue(
    ...
    background_tasks.add_task(
        update_issue, response_dict["response"], issue_data.issue.id, description
    )
    return "OK"
...
async def query_ai(summary: str, description: str):
    chat = ChatOpenAI(
        model_name="gpt-3.5-turbo", temperature=0.5, openai_api_key=open_ai_token
    )
    ...
    try:
        response = chat(
            [SystemMessage(content=system_m), HumanMessage(content=question)]
        )
